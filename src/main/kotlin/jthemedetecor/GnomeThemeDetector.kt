...
internal class GnomeThemeDetector : OsThemeDetector() {
    ...
    private val darkThemeNamePattern: Pattern = Pattern.compile(".*dark.*", Pattern.CASE_INSENSITIVE)
    ...
    private class DetectorThread(private val detector: GnomeThemeDetector) : Thread() {
        private val outputPattern: Pattern = Pattern.compile("(gtk-theme|color-scheme).*", Pattern.CASE_INSENSITIVE)
            ...
            try {
                ...
                BufferedReader(InputStreamReader(monitoringProcess.inputStream)).use { reader ->
                    while (!this.isInterrupted) {
                        ...
                        val keyValue = readLine.split("\\s".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
                        ...
                        if (currentDetection != lastValue) {
                            ...
                            for (listener in detector.listeners) {
                                when (listener) {
                                    is DarkModeConsumer -> listener.accept(currentDetection)
                                    is PrimaryColorConsumer -> throw RuntimeException("Not Implemented")
                                }
                            }
                        }
                    }
