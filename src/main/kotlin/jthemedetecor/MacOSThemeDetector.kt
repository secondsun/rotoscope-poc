...
internal class MacOSThemeDetector : OsThemeDetector() {
    ...
    private val callbackExecutor: ExecutorService = Executors.newSingleThreadExecutor { runnable: Runnable ->
        DetectorThread(
            runnable
        )
    }
    ...
    private fun initObserver() {
        ...
        try {
            val delegateClass =
                Foundation.allocateObjcClassPair(Foundation.getObjcClass("NSObject"), "NSColorChangesObserver")
    ...
    override val isDark: Boolean
        get() {
            ...
            try {
                val userDefaults =
                    Foundation.invoke("NSUserDefaults", "standardUserDefaults")
    ...
    private fun notifyListeners(primaryColor: Color) {
        listeners.stream().filter { it: ThemingConsumer<*>? -> it is PrimaryColorConsumer }
            .map { it: ThemingConsumer<*>? -> it as PrimaryColorConsumer }
            .forEach { listener: PrimaryColorConsumer -> listener.accept(primaryColor) }
    }

    private fun notifyListeners(isDark: Boolean) {
        listeners.stream().filter { it: ThemingConsumer<*>? -> it is DarkModeConsumer }
            .map { it: ThemingConsumer<*>? -> it as DarkModeConsumer }
            .forEach { listener: DarkModeConsumer -> listener.accept(isDark) }
    }
